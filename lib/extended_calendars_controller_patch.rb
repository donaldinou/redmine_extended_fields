    ...
    module InstanceMethods
        ...
        def extended_show
            ...
            if @query.valid?
                ...
                events += CustomValue.all.joins(:custom_field)
                                         .where([ "#{CustomField.table_name}.field_format = ? AND #{CustomValue.table_name}.customized_type = ? AND #{CustomValue.table_name}.customized_id IN (?) AND STR_TO_DATE(#{CustomValue.table_name}.value, '%Y-%m-%d') BETWEEN ? AND ?",
                                                  'date', 'Issue', @query.issue_ids, @calendar.startdt, @calendar.enddt ])
